...
[RequireComponent(typeof(BossFrog))]
public class LastWord_BossFrog : EntityPattern {
    [SerializeField] private BossFrog bossObject;
        ...
        while (sequenceDelay > 0f)
        {
            if (timerCollective >= timePerSummon)
            {
                for (int i = 0; i < Mathf.FloorToInt(timerCollective / timePerSummon); i++)
                {
                    ...
                    Instantiate(firingShip, summoner.transform.position, Quaternion.identity);
